...
# CH will broadcost SCH packets to all it's members
...
# Each member which has data will send ctrl packet to CH and each member which has data will start listning from the begining of control period
	...
	for i in range(len(clusterList)):
		...
		for j in range(len(clusterList[i])):
			...
			if(clusterList[i][j]['isClusterHead']==False and clusterList[i][j]['isDead']==False and math.floor(random.random()/0.5)==1):
				...
				new_count_=0
				for k in range(j):
					if(clusterList[i][k]['wantToSendData']==True and clusterList[i][k]['isDead']==False):
						new_count_+=1
				clusterList[i][j]['energy']-=new_count_*recievingEnergy(k)
				clusterList[i][j]['energy']-=(j-new_count_)*(recievingEnergy(k))/2
				if(clusterList[i][j]['energy']<=0):
					clusterList[i][j]['energy']=0
					clusterList[i][j]['isDead']=True
		for j in range(len(clusterList[i])):
...
file_name='output_bee_mac100.txt'
...
roundAxis_new = np.linspace(roundAxis.min(), roundAxis.max(),80000)
numberOfDeadNodesAxis_smooth=spline(roundAxis,numberOfDeadNodesAxis,roundAxis_new)
plt.text(firstDeadNodeRound-100, -5, firstDeadNodeRound)
plt.text(lastDeadNodeRound-100, 105, lastDeadNodeRound)
plt.plot(roundAxis_new,numberOfDeadNodesAxis_smooth,'r',label='BEE-MAC')
